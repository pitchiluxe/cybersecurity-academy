...
- **3D labs** (`/labs/*`, Three.js via `next/dynamic({ ssr: false })` — never import R3F in jest): wiring lab (`lib/wiringLab.ts` validation/scoring `max(60, 100 - 10·wrong)`, `components/lab/WiringScene.tsx` scene, 3 static fallback scenarios) and two cable-then-configure CLI labs sharing `components/lab/DeviceCliLab.tsx` (wiring phase + simulated device CLI; pages are thin config wrappers): FortiGate (`lib/fortigateLab.ts`, FortiOS CLI) and Cisco router (`lib/routerLab.ts`, IOS-XE CLI — reuses the FortiGate scenario shape/parser/`[TASK_DONE:id]`/`[LAB_COMPLETE]` markers, score `max(60, 100 - 5·wrong - 5·hints)`). Routes `/api/lab/wiring`, `/api/lab/{fortigate,router}/{init,exec}` fall back to canned scenarios so labs work under rate limits; `/api/lab/complete` records `ticket_results` (wiring→network, fortigate→firewall, router→network) and re-checks certs. Three deps pinned for React 18: `@react-three/fiber@8`, `@react-three/drei@9`, `three@0.160`.
- **Lab job board**: `/labs` is a client page that fetches an AI-generated catalog of 5–10 random lab briefs (`lib/labCatalog.ts`, `POST /api/lab/list`, 10-brief canned fallback) and caches it in `sessionStorage` (`labs:catalog`; "New job board" button refetches). Each brief maps to an engine (`wiring`/`fortigate`/`router`); clicking a card stashes it (`lab:<id>`) and opens `/labs/<engine>?lab=<id>`, whose scenario prompt is seeded with the brief (`readStashedBrief`). Without the param, labs generate a fresh random scenario as before.
